...
Create list of samples (last one being the Wilson study):
...
Create merge order (oldest to youngest embryo; most cells to least cells; Wilson study):
...
```{r}
mnn <- do.call(noCorrect, all)

counts(mnn) <- do.call(cbind, lapply(all, counts))

sizeFactors(mnn) <- unlist(lapply(all, sizeFactors))

logcounts(mnn) <- do.call(cbind, lapply(all, logcounts))

assay(mnn, "merged") <- NULL
```
...
mnn$study[mnn$batch == "x"] <- "Wilson"
```

### kNN

Identify kNN cells for Wilson study:

```{r}
dim <- list(
  qry = reducedDim(mnn, "corrected")[mnn$study == "Wilson", ],
  sbj = reducedDim(mnn, "corrected")[mnn$study == "Marioni", ]
)

rownames(dim$qry) <- colnames(mnn)[mnn$study == "Wilson"]
...
Annotate cells with cell type label:
...
colData(mnn)[rownames(dim$qry), "celltype"] <- val
```
...
Use UMAP settings from MouseGastrulationData analysis:
